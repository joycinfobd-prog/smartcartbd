...
import { MobileDarazHome } from './components/MobileDarazHome';
...
export default function App() {
  ...
  // Live-pull support conversations so replies from the support team appear
  // without a page reload (chat widget + staff helpdesk both stay fresh).
  const inquiriesJsonRef = useRef('');
  useEffect(() => {
    let alive = true;
    ...
    const pull = async () => {
      try {
        ...
        if (!alive || !Array.isArray(data.inquiries)) return;
        const server = data.inquiries as SupportInquiry[];
        setInquiriesList((prev) => {
          const map = new Map(server.map((i) => [i.id, i]));
          prev.forEach((i) => {
            if (!map.has(i.id)) map.set(i.id, i);
          });
          const next = Array.from(map.values());
          const json = JSON.stringify(next);
          if (json === inquiriesJsonRef.current) return prev;
          inquiriesJsonRef.current = json;
          return next;
        });
      } catch {
        /* offline / sleeping backend — keep local state */
      }
    };
  ...
  const handleReplyInquiry = (
    ...
    setInquiriesList(prev => prev.map(inq => {
      if (inq.id === inquiryId) {
        // If a customer is sending a follow-up, move ticket back to open/in_progress
        const nextStatus = isStaff
          ? (inq.status === 'open' ? 'in_progress' : inq.status)
          : (inq.status === 'resolved' ? 'open' : inq.status);
        return {
          ...inq,
          status: nextStatus,
          assignedTo: isStaff ? responderName : inq.assignedTo,
  ...
  // Support entry point: staff get the helpdesk, visitors get the live chat box
  ...
  const handleCreateInquiry = (inqData: Partial<SupportInquiry>) => {
    ...
    // Remember this ticket in this browser so the visitor can come back and
    // read the support team's reply (guests included).
    try {
      const mine: string[] = JSON.parse(
        localStorage.getItem('smartcart_my_inquiries') || '[]',
      );
      if (!mine.includes(newInquiry.id)) mine.push(newInquiry.id);
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden bg-[#f8fafc] text-slate-800 selection:bg-emerald-500 selection:text-white">
      ...
      {/* Main Container */}
      <main>
        {/* Mobile-only Daraz-style home (tiles, flash sale, category rail) */}
        <MobileDarazHome
          products={products}
          currency={currency}
          onSelectCategory={setActiveCategory}
          onViewProduct={(p) => setSelectedProductForDetails(p)}
          onOpenSupport={handleOpenSupport}
          onOpenWishlist={() => setIsWishlistOpen(true)}
          onOpenTracking={() => handleOpenTrackingWithCode()}
        />

        {/* Dynamic Hero Section (desktop & tablet) */}
        ...
        {/* Interactive Categories Bar (desktop & tablet) */}
        <div className="hidden sm:block">
          <CategoriesSection
        ...
        {/* Flash Deals / Countdown (desktop & tablet) */}
        <section id="deals" className="hidden pt-2 sm:block">
          <DealsCountdown
      ...
      {/* Floating Action Buttons: WhatsApp hotline + Support inbox side-by-side */}
      <FloatingWhatsApp
        storeSettings={storeSettings}
        currentUser={currentUser}
        onOpenSupport={handleOpenSupport}
        supportBadge={
          currentUser && currentUser.role !== 'customer'
            ? inquiriesList.filter((i) => i.status === 'open').length
            : chatUnread
        }
      />

      {/* Mobile bottom tab bar (Daraz-style quick nav) */}
      <MobileBottomBar
        cartCount={cartCount}
        supportBadge={
          currentUser && currentUser.role !== 'customer'
      ...
      {/* 2.5 LIVE CHAT BOX — customer ↔ support team conversation on-page */}
      <LiveChatWidget
        isOpen={isLiveChatOpen}
        onClose={() => setIsLiveChatOpen(false)}
        inquiries={inquiriesList}
        currentUser={currentUser}
        onCreateThread={({ name: n, phone: ph, message }) => {
          handleCreateInquiry({
            customerName: n,
            phone: ph,
            subject: message.length > 40 ? message.slice(0, 37) + '...' : message,
            message,
            category: 'General',
            priority: 'normal',
            status: 'open',
          ...
          setIsLiveChatOpen(true);
          showToast('চ্যাট শুরু হয়েছে — সাপোর্ট টিম শীঘ্রই উত্তর দেবে।');
        }}
        onSendFollowUp={(threadId, text, senderName) => {
          handleReplyInquiry(threadId, text, senderName, false);
        }}
      ...
      <CustomerSupportModal
        ...
        inquiries={inquiriesList}
        notifications={notificationsList}
        onAddReply={(inqId, text, senderName, isStaff) => {
          handleReplyInquiry(
            inqId,
            text,
            senderName || currentUser?.fullName || (isStaff ? 'Customer Support' : 'Customer'),
            isStaff,
          );
        }}
