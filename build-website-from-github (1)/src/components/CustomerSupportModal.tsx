...
interface CustomerSupportModalProps {
  ...
  onAddReply: (inquiryId: string, text: string) => void;
...
export const CustomerSupportModal: React.FC<CustomerSupportModalProps> = ({
  ...
  const activeInquiry = safeInquiries.find((inq) => inq.id === selectedInquiryId) || safeInquiries[0] || null;

  // Filter support relevant notifications
  ...
  const handleSendReply = (e: React.FormEvent) => {
    ...
    onAddReply(activeInquiry.id, replyText.trim());
    setReplyText('');
    setSuccessMsg('Reply sent to customer ticket!');
...
দয়া করে আপনার বিস্তারিত বলুন। আমরা দ্রুত সহায়তা করছি।`;
  ...
  const handleCreateNewTicketSubmit = (e: React.FormEvent) => {
    ...
    if (onCreateInquiry) {
      onCreateInquiry({
        ...
        priority: 'normal',
        status: 'open',
        assignedTo: currentUser?.fullName || 'Rimi Akter'
      });
    }
    ...
    setNewCustSubject('');
    setNewCustMessage('');
    setSuccessMsg('New support ticket recorded successfully!');
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/80 backdrop-blur-xs overflow-y-auto">
      ...
      >
        {/* Top Support Portal Header */}
        <div className="bg-gradient-to-r from-sky-600 via-sky-700 to-slate-900 p-5 text-white flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-2xl bg-white text-sky-700 flex items-center justify-center font-black shadow-md shrink-0">
              <Headphones size={26} />
            </div>
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <span className="text-[10px] font-black uppercase tracking-widest px-2.5 py-0.5 rounded-full bg-black/20 text-sky-200 border border-sky-300/30">
        ...
        <div className="bg-sky-50 border-b border-sky-200 px-5 py-2 flex items-center justify-between gap-3 text-xs text-sky-900">
        ...
        <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 overflow-hidden">
          {/* Left Column: Tickets Queue (5 columns) */}
          ...
          {/* Right Column: Active Conversation & Helpdesk Station (7 columns) */}
          <div className="lg:col-span-7 flex flex-col overflow-y-auto p-4 sm:p-6 bg-white">
            ...
            {isCreatingNew ? (
              ...
              <div className="space-y-4">
                ...
                <div className="bg-sky-50/60 border border-sky-200 rounded-2xl p-3.5 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="py-2 px-3.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs flex items-center gap-1.5 shadow-xs transition"
                    >
                ...
                {/* Quick Canned Preset Responses in Bengali & English */}
                ...
                <form onSubmit={handleSendReply} className="space-y-2 pt-2">
                  <div className="relative">
                    <textarea
                      ...
                      placeholder={`Reply to ${activeInquiry.customerName} as ${currentUser?.fullName || 'Support Staff'}...`}
                      rows={3}
                      className="w-full p-3 rounded-2xl border border-slate-200 text-xs font-medium text-slate-800 focus:border-sky-600 outline-none pr-12"
                      required
                    />
                    <button
                      type="submit"
                      className="absolute right-3 bottom-3 p-2 rounded-xl bg-sky-600 hover:bg-sky-700 text-white transition shadow-xs cursor-pointer"
                      title="Send response"
                    >
                      <Send size={15} />
                    </button>
